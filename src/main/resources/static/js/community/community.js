$(document).ready(function () {
   let communityTable =  $('#community_table').DataTable({
        ...
        ajax:{
            type:"POST",
            url: "/admin/community/selectList",
        },
        columns:[
            {data:null},
            {data:"id"},
    ...
    $("#checkall").click(function(){
        if($(this).prop("checked")){
            communityTable.rows().select();
            $(".select-checkbox").addClass('selected');
        }else{
            communityTable.rows().deselect();
    ...
    //     const numCols = communityTable.columns().nodes().length;
    //
    //     //검색 시 기존의 검색된 데이터들을 초기화 시킴
    //     for(let i=0; i<numCols; i++) {
    //         communityTable.columns(i).search('');
    //     }
    //
    //     const searchType = $("#searchType").val();
    //     const searchValue = $("#searchInput").val();
    //
    //     communityTable.column(searchType).search(searchValue).draw();
    // })
...
const searchBtn = () => {
    ...
    const table = $('#community_table').DataTable();
...
const handleDelete = () => {
        //체크 된 로우의 id값을 가져옴
        let row_data = $.map($('#community_table').DataTable().rows('.selected').data(), function(item){
            return item.id;
        });
